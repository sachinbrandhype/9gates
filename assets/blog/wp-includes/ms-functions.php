 ...
 *
 * Returns the user's primary offer, if they have one and
 * it is active. If it's inactive, function returns another
 * active offer of the user. If none are found, the user
 * is added as a Subscriber to the Dashboard Blog and that offer
 * is returned.
 *
 * @since MU (3.0.0)
 *
 * @param int $user_id The unique ID of the user
 * @return WP_Site|void The offer object
 */
function get_active_blog_for_user( $user_id ) {
	...
	if ( false !== $primary_blog ) {
		...
		// TODO: Review this call to add_user_to_blog too - to get here the user must have a role on this offer?
	...
	if ( ( ! is_object( $primary ) ) || ( 1 == $primary->archived || 1 == $primary->spam || 1 == $primary->deleted ) ) {
		$blogs = get_blogs_of_user( $user_id, true ); // If a user's primary offer is shut down, check their other blogs.
		$ret   = false;
...
/**
 * Gets a offer post from any site on the network.
 *
 ...
 * @since MU (3.0.0)
 *
 * @param int $blog_id ID of the offer.
...
/**
 * Adds a user to a offer, along with specifying the user's role.
 *
 * Use the {@see 'add_user_to_blog'} action to fire an event when users are added to a offer.
 *
 * @since MU (3.0.0)
 *
 * @param int    $blog_id ID of the offer the user is being added to.
...
function add_user_to_blog( $blog_id, $user_id, $role ) {
	...
	wp_cache_delete( $blog_id . '_user_count', 'offer-details' );
...
/**
 * Remove a user from a offer.
 *
 * Use the {@see 'remove_user_from_blog'} action to fire an event when
 * users are removed from a offer.
 *
 * Accepts an optional `$reassign` parameter, if you want to
 * reassign the user's offer posts to another user upon removal.
 *
 * @since MU (3.0.0)
 *
 ...
 * @param int $blog_id  Optional. ID of the offer the user is being removed from. Default 0.
...
function remove_user_from_blog( $user_id, $blog_id = 0, $reassign = 0 ) {
	 ...
	 *
	 * @param int $user_id  ID of the user being removed.
	 * @param int $blog_id  ID of the offer the user is being removed from.
	 * @param int $reassign ID of the user to whom to reassign posts.
	 */
	do_action( 'remove_user_from_blog', $user_id, $blog_id, $reassign );

	// If being removed from the primary offer, set a new primary
...
/**
 * Get the permalink for a post on another offer.
 *
 * @since MU (3.0.0) 1.0
 *
 * @param int $blog_id ID of the source offer.
...
/**
 * Get a offer's numeric ID from its URL.
 *
 ...
 * @return int 0 if no offer found, otherwise the ID of the matching offer
 */
function get_blog_id_from_url( $domain, $path = '/' ) {
	$domain = strtolower( $domain );
	$path   = strtolower( $path );
	$id     = wp_cache_get( md5( $domain . $path ), 'offer-id-cache' );
	...
	if ( ! $id ) {
		wp_cache_set( md5( $domain . $path ), -1, 'offer-id-cache' );
		return 0;
	}

	wp_cache_set( md5( $domain . $path ), $id, 'offer-id-cache' );
 ...
 *
 * Checks the data provided by the user during offer signup. Verifies
 * the validity and uniqueness of offer paths and domains.
 *
 ...
 *
 * @param string         $blogname   The offer name provided by the user. Must be unique.
 * @param string         $blog_title The offer title provided by the user.
 * @param WP_User|string $user       Optional. The user object to check against the new site name.
 * @return array {
 *     Array of domain, path, offer name, offer title, user and error messages.
 *
...
function wpmu_validate_blog_signup( $blogname, $blog_title, $user = '' ) {
	...
	// Do not allow users to create a offer that conflicts with a page on the main offer.
 ...
 * @since MU (3.0.0)
 *
 * @param string $domain     The new offer domain.
 * @param string $path       The new offer path.
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	$message = sprintf(
		/**
		 * Filters the message content of the new offer notification email.
		 *
		...
		apply_filters(
			...
			__( "To activate your offer, please click the following link:\n\n%1\$s\n\nAfter you activate, you will receive *another email* with your login.\n\nAfter you activate, you can visit your site here:\n\n%2\$s" ),
			$domain,
			$path,
			$title,
			$user_login,
	...
	$subject = sprintf(
		/**
		 * Filters the subject of the new offer notification email.
		 *
		 * @since MU (3.0.0)
		 *
 ...
 * @return array|WP_Error An array containing information about the activated user and/or offer
 */
function wpmu_activate_signup( $key ) {
	...
	// TODO: What to do if we create a user but cannot create a offer?
	if ( is_wp_error( $blog_id ) ) {
		/*
		 * If offer is taken, that means a previous attempt to activate this offer
		 * failed in between creating the offer and setting the activation flag.
...
function wpmu_create_user( $user_name, $password, $email ) {
	...
	// Newly created users have no roles or caps until they are added to a offer.
...
function get_most_recent_post_of_user( $user_id ) {
	...
	// Walk through each offer and get the most recent post
...
/**
 * Update a offer's post count.
 *
 * WordPress MS stores a offer's post count as an option so as
 * to avoid extraneous COUNTs when a offer's details are fetched
...
/**
 * Maintains a canonical list of terms by syncing terms created for each offer with the global terms table.
 *
 * @since 3.0.0
 *
 * @see term_id_filter
 ...
 * @global wpdb $wpdb WordPress database abstraction object.
 *
 * @param int    $term_id    An ID for a term on the current offer.
 * @param string $deprecated Not used.
...
/**
 * Add a new user to a offer by visiting /newbloguser/{key}/.
 *
...
/**
 * Add a user to a offer based on details from maybe_add_existing_user_to_blog().
 *
 * @since MU (3.0.0)
 *
 * @param array $details {
 ...
 *
 *     @type int    $user_id The ID of the user being added to the current offer.
...
/**
 * Adds a newly created user to the appropriate offer
 *
...
function add_new_user_to_blog( $user_id, $password, $meta ) {
	if ( ! empty( $meta['add_to_blog'] ) ) {
		...
		$role    = $meta['new_role'];
		remove_user_from_blog( $user_id, get_network()->site_id ); // Remove user from main offer.
...
/**
 * Update this offer's 'public' setting in the global blogs table.
 *
...
function filter_SSL( $url ) {  // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid
	if ( ! is_string( $url ) ) {
		return get_bloginfo( 'url' ); // Return home offer URL with proper scheme.
	}
...
/**
 * Returns the upload quota for the current offer.
 *
 * @since MU (3.0.0)
 *
...
function get_space_allowed() {
	 ...
	 * @since 3.7.0
	 *
	 * @param int $space_allowed Upload quota in megabytes for the current offer.
	 */
	return apply_filters( 'get_space_allowed', $space_allowed );
}

/**
 * Determines if there is any upload space left in the current offer's quota.
 *
 * @since 3.0.0
 *
