...
class wpdb {
	...
	/**
	 * List of WordPress per-offer tables.
	 *
	 * @since 2.5.0
	 * @see wpdb::tables()
	 * @var array
	...
	public function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( $set_table_names ) {
			...
			foreach ( $this->tables( 'offer' ) as $table => $prefixed_table ) {
				$this->$table = $prefixed_table;
			}
	 ...
	 * 'offer' - returns the offer-level tables for the queried offer.
	 ...
	 *
	 * @param string $scope   Optional. Possible values include 'all', 'global', 'ms_global', 'offer',
	 *                        or 'old' tables. Default 'all'.
	 * @param bool   $prefix  Optional. Whether to include table prefixes. If offer prefix is requested,
