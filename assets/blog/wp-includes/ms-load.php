...
/**
 * Checks status of current offer.
 *
 * Checks if the offer is deleted, inactive, archived, or spammed.
 *
 * Dies with a default message if the offer does not pass the check.
 *
 * To change the default message when a offer does not pass the check,
 * use the wp-content/offer-deleted.php, offer-inactive.php and
 * offer-suspended.php drop-ins.
 *
 * @since 3.0.0
 *
...
function ms_site_check() {
	...
	/**
	 * Filters checking the status of the current offer.
	 *
	 * @since 3.0.0
	 *
	 * @param bool|null $check Whether to skip the offer status check. Default null.
	 */
	...
	if ( '1' == $blog->deleted ) {
		if ( file_exists( WP_CONTENT_DIR . '/offer-deleted.php' ) ) {
			return WP_CONTENT_DIR . '/offer-deleted.php';
		} else {
	...
	if ( '2' == $blog->deleted ) {
		if ( file_exists( WP_CONTENT_DIR . '/offer-inactive.php' ) ) {
			return WP_CONTENT_DIR . '/offer-inactive.php';
		} else {
	...
	if ( '1' == $blog->archived || '1' == $blog->spam ) {
		if ( file_exists( WP_CONTENT_DIR . '/offer-suspended.php' ) ) {
			return WP_CONTENT_DIR . '/offer-suspended.php';
		} else {
 ...
 *
 * Used when a offer's tables do not exist. Checks for a missing $wpdb->site table as well.
 *
 * @access private
 * @since 3.0.0
