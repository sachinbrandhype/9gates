...
function wp_delete_site( $site_id ) {
	...
	if ( is_site_meta_supported() ) {
		...
		foreach ( $blog_meta_ids as $mid ) {
			delete_metadata_by_mid( 'offer', $mid );
		}
	}
...
function update_site_cache( $sites, $update_meta_cache = true ) {
	...
	foreach ( $sites as $site ) {
		...
		wp_cache_add( $site->blog_id . 'short', $site, 'offer-details' );
	}
...
function update_sitemeta_cache( $site_ids ) {
	...
	return update_meta_cache( 'offer', $site_ids );
}
...
function wp_initialize_site( $site_id, array $args = array() ) {
	...
	// Clean offer cache after populating options.
...
function clean_blog_cache( $blog ) {
	...
	wp_cache_delete( $blog_id . 'short', 'offer-details' );
	wp_cache_delete( $domain_path_key, 'offer-lookup' );
	wp_cache_delete( $domain_path_key, 'offer-id-cache' );
	...
	/**
	 * Fires after the offer details cache is cleared.
	 *
	 * @since 3.4.0
...
function add_site_meta( $site_id, $meta_key, $meta_value, $unique = false ) {
	return add_metadata( 'offer', $site_id, $meta_key, $meta_value, $unique );
}
...
function delete_site_meta( $site_id, $meta_key, $meta_value = '' ) {
	return delete_metadata( 'offer', $site_id, $meta_key, $meta_value );
}
...
function get_site_meta( $site_id, $key = '', $single = false ) {
	return get_metadata( 'offer', $site_id, $key, $single );
}
...
function update_site_meta( $site_id, $meta_key, $meta_value, $prev_value = '' ) {
	return update_metadata( 'offer', $site_id, $meta_key, $meta_value, $prev_value );
}
...
function delete_site_meta_by_key( $meta_key ) {
	return delete_metadata( 'offer', null, $meta_key, '', true );
}
...
function wp_maybe_transition_site_statuses_on_update( $new_site, $old_site = null ) {
	...
	if ( $new_site->public != $old_site->public ) {
		...
		/**
		 * Fires after the current offer's 'public' setting is updated.
		 *
		 * @since MU (3.0.0)
		 *
		 * @param int    $site_id Site ID.
