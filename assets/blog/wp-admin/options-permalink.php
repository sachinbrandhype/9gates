...
get_current_screen()->add_help_tab(
	array(
		'id'      => 'overview',
		'title'   => __( 'Overview' ),
		'content' => '<p>' . __( 'Permalinks are the permanent URLs to your individual pages and offer posts, as well as your category and tag archives. A permalink is the web address used to link to your content. The URL to each post should be permanent, and never change &#8212; hence the name permalink.' ) . '</p>' .
...
/*
 * In a subdirectory configuration of multisite, the `/blog_okok` prefix is used by
 ...
 */
if ( is_multisite() && ! is_subdomain_install() && is_main_site() && 0 === strpos( $permalink_structure, '/blog_okok/' ) ) {
	$blog_prefix = '/offer';
}
...
<?php
if ( is_multisite() && ! is_subdomain_install() && is_main_site() && 0 === strpos( $permalink_structure, '/blog_okok/' ) ) {
	$permalink_structure = preg_replace( '|^/?offer|', '', $permalink_structure );
	$category_base       = preg_replace( '|^/?blog_okok|', '', $category_base );
	$tag_base            = preg_replace( '|^/?blog_okok|', '', $tag_base );
}
