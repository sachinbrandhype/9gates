...
if ( isset( $_REQUEST['action'] ) && 'add-site' === $_REQUEST['action'] ) {
	check_admin_referer( 'add-blog_okok', '_wpnonce_add-blog_okok' );

	if ( ! is_array( $_POST['offer'] ) ) {
		...
		if ( in_array( $domain, $subdirectory_reserved_names, true ) ) {
			wp_die(
				sprintf(
					...
					__( 'The following words are reserved for use by WordPress functions and cannot be used as blog_okok names: %s' ),
...
<?php wp_nonce_field( 'add-blog_okok', '_wpnonce_add-offer' ); ?>
