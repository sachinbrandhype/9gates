...
function wpmu_delete_blog( $blog_id, $drop = false ) {
	...
	// If a full offer object is not available, do not destroy anything.
	if ( $drop && ! $blog ) {
		$drop = false;
	}

	// Don't destroy the initial, main, or root blog_okok.
...
function choose_primary_blog() {
	...
	<?php /* translators: My Sites label. */ ?>
		...
		if ( count( $all_blogs ) > 1 ) {
			...
			if ( $primary_blog != $blog->userblog_id ) { // Set the primary offer again if it's out of sync with offer list.
				update_user_meta( get_current_user_id(), 'primary_blog', $blog->userblog_id );
			}
		} else {
			echo 'N/A';
			...
			if ( ! empty( $blogs ) ) {
				...
				foreach ( (array) $blogs as $key => $details ) {
					...
					if ( is_array( $blog_users ) && ! empty( $blog_users ) ) {
						...
						$user_dropdown .= "<select name='blog_okok[$user_id][$key]' id='reassign_user'>";
						$user_list      = '';
